...
export default function customErrorMessages(err: any, _customOptions?: any) {
  switch (err.type) {
    ...
    case "string.min":
      ...
      err.message = `${codeErrors.STRING.ATTRIBUTES.MIN.message} ${err.context.limit}`;
      break;
    case "string.max":
      err.type = codeErrors.STRING.ATTRIBUTES.MAX.code;
      err.message = `${codeErrors.STRING.ATTRIBUTES.MAX.message} ${err.context.limit}`;
      break;
